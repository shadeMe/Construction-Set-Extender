...
// copy of GameBSExtraData.h/cpp with a few new methods for BaseExtraList
...
struct BaseExtraList
{
	...
	// new methods
	BSExtraData*	ModExtraCharge(float charge);
	BSExtraData*	ModExtraCount(UInt32 count);
	BSExtraData*	ModExtraEnableStateParent(TESObjectREFR* parent);
	BSExtraData*	ModExtraEnableStateParentOppositeState(bool oppositeState);
	BSExtraData*	ModExtraGlobal(TESGlobal* global);
	BSExtraData*	ModExtraHealth(float health);
	BSExtraData*	ModExtraOwnership(TESForm* owner);
	BSExtraData*	ModExtraRank(int rank);
	BSExtraData*	ModExtraSoul(UInt32 soul);
	BSExtraData*	ModExtraTimeLeft(float timeLeft);
