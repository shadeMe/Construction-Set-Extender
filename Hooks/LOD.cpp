...
namespace Hooks
{
	...
	void __stdcall ShowLODTextureGenNotification(bool State)
	{
		...
		{
			s_NotificationDialog = CreateDialogParam(g_DLLInstance, MAKEINTRESOURCE(DLG_IDLE), *g_HWND_CSParent, NULL, NULL);
			...
			s_LODDiffuseMapPartialResolution = g_INIManager->GetINIInt("PartialTextureResolution", "Extender::LOD");
		...
		{
			DebugPrint("Generated %d partial diffuse maps in total", s_NotificationMapCounter);
			...
			if (g_INIManager->GetINIInt("DeletePartialsAfterGeneration", "Extender::LOD"))
			{
				char Buffer[MAX_PATH] = {0};
				sprintf_s(Buffer, sizeof(Buffer), "%sData\\Textures\\LandscapeLOD\\Generated\\Partial\\", g_APPPath.c_str());
				...
				if ((OpResult = SHFileOperation(&DeleteShellOp)))
					DebugPrint("Couldn't delete landscape LOD partial texture directory. Error: %04X", OpResult);
			}
		}
	}
	...
	void __stdcall DoLODTextureGenBlackTextureFixHook(NiNode* RenderNode, NiCamera* Camera)
	{
		...
		if (AUXVIEWPORT->IsHidden() == false && AUXVIEWPORT->IsFrozen() == false)
		{
