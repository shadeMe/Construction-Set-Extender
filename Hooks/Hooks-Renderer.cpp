...
namespace cse
{
	...
	//		_MemHdlr(TestHook1).WriteJump();
		...
		{
			// we only need to handle Alt (and special-case Shift) here as the rest are handled by the input manager
			switch (vKey)
			{
			case VK_MENU:
				{
			...
			case VK_SHIFT:
				...
				break;
			default:
				SME_ASSERT(!"Invalid virtual key!");
			}
		...
		_hhBegin()
		{
			_hhSetVar(Retn, 0x00427593);
			__asm
			{
				// skip over the consistency checks for the holdable keys
				// however, the renderer uses the Shift key's state when placing pathgrid points to set the 'autogenerated' flag (why?)
				// we need to preserve this behaviour
				// ### TODO broken! - the flag gets reset elsewhere, investigate
				mov		esi, GetAsyncKeyStateOverride

				push	VK_SHIFT
				call	esi
				mov		byte ptr [esp + 0x47], al
