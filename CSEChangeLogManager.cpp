...
namespace ConstructionSetExtender
{
	namespace ChangeLogManager
	{
		...
		const char* CSEChangeEntry::Get() const
		{
			return Buffer.c_str();
		}

		CSEBasicFormChangeEntry::CSEBasicFormChangeEntry( UInt8 ChangeType, TESForm* Form, UInt32 Value ) :
			CSEChangeEntry()
		{
			SME_ASSERT(Form);
		...
		CSEBasicFormChangeEntry::~CSEBasicFormChangeEntry()
		{
			;//
		}
