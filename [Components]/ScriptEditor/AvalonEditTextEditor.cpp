...
#pragma region Methods
	...
	AvalonEditHighlightingDefinition^ AvalonEditTextEditor::CreateSyntaxHighlightDefinitions( void )
	{
		...
		bool BoldFaced = PREFERENCES->FetchSettingAsInt("BoldFacedHighlighting");
		...
		return SyntaxHighlightingManager->CreateDefinitionFromSerializedData("ObScript");
	}
#pragma endregion
...
#pragma region Event Handlers
	...
	void AvalonEditTextEditor::ScriptEditorPreferences_Saved( Object^ Sender, EventArgs^ E )
	{
		...
		if (PREFERENCES->FetchSettingAsInt("CodeFolding"))
			CodeFoldingStrategy = gcnew AvalonEditObScriptCodeFoldingStrategy();
		TextField->Options->CutCopyWholeLine = PREFERENCES->FetchSettingAsInt("CutCopyEntireLine");
		TextField->Options->ShowSpaces = PREFERENCES->FetchSettingAsInt("ShowSpaces");
		TextField->Options->ShowTabs = PREFERENCES->FetchSettingAsInt("ShowTabs");
		TextField->WordWrap = PREFERENCES->FetchSettingAsInt("WordWrap");
		if (PREFERENCES->FetchSettingAsInt("AutoIndent"))
			TextField->TextArea->IndentationStrategy = gcnew AvalonEditObScriptIndentStrategy(true, true);
		IntelliSenseBox->MaximumVisibleItemCount = PREFERENCES->FetchSettingAsInt("MaxVisibleItems");
...
AvalonEditTextEditor::AvalonEditTextEditor(Font^ Font, Object^ Parent)
{
	...
	LocalVarsDatabaseUpdateTimer->Tick += gcnew EventHandler(this, &AvalonEditTextEditor::LocalVarsDatabaseUpdateTimer_Tick);
	...
	PREFERENCES->PreferencesSaved += gcnew EventHandler(this, &AvalonEditTextEditor::ScriptEditorPreferences_Saved);
}