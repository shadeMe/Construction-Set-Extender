...
#define IDC_CSEFILTERABLEFORMLIST_FORMID			9013
		...
		RenderWindowMiscData::RenderWindowMiscData() :
			bgsee::WindowExtraData(kTypeID)
		{
			...
			case WM_COMMAND:
				...
				case IDC_RENDERWINDOWCONTEXT_UNGROUP:
					if (_RENDERSEL->selectionCount > 1)
					{
						...
						{
						case IDC_RENDERWINDOWCONTEXT_GROUP:
							if (RenderWindowGroupManager::Instance.AddGroup(_RENDERSEL) == false)
							{
							...
							break;
						case IDC_RENDERWINDOWCONTEXT_UNGROUP:
							if (RenderWindowGroupManager::Instance.RemoveGroup(_RENDERSEL) == false)
							{
								BGSEEUI->MsgBoxW(hWnd, 0,
...
#define IDT_RENDERWINDOW_TITLEBARUPDATE			0x154
													...
													bool& Return, bgsee::WindowExtraDataCollection* ExtraData )
		{
			...
			case WM_INITDIALOG:
				{
					RenderWindowMiscData* xData = BGSEE_GETWINDOWXDATA(RenderWindowMiscData, ExtraData);
					if (xData == NULL)
					{
						xData = new RenderWindowMiscData();
						ExtraData->Add(xData);
					}
			...
			case WM_DESTROY:
				{
					...
					if (xData)
					{
						ExtraData->Remove(RenderWindowMiscData::kTypeID);
						delete xData;
					}
			...
			case WM_TIMER:
				...
				case TESRenderWindow::kTimer_ViewportUpdate:			// update timer
					...
					if (bgsee::RenderWindowFlyCamera::FlyCamModeActive == false)
					{
			...
			case WM_MOUSEMOVE:
				{
					...
					if (GetCapture() != hWnd && bgsee::RenderWindowFlyCamera::FlyCamModeActive == false)
					{
