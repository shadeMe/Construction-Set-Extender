...
namespace bgsee
{
	...
	static const std::unordered_map<int, const char*> kWindowMessageStringTable = {
		{0, "WM_NULL" },
		{1, "WM_CREATE" },
		{2, "WM_DESTROY" },
		{3, "WM_MOVE" },
	...
	std::string WindowMessageToString(UINT uMsg, bool AppendHexValue)
	{
		...
		if (Match == kWindowMessageStringTable.end())
			Out = "<unknown_window_message>";
		else
			Out = Match->second;

		if (AppendHexValue)
			Out += " (" + std::string(HexStringBuffer) + ")";
