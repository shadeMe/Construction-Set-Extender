...
namespace CSAutomationScript
{
	void RegisterFormCommands()
	{
		...
		REGISTER_CSASCOMMAND(GetDataHandlerFormList, "Form List Functions");
	}
	...
	template <typename T> void AddFormToCSASArray(tList<T>* List, std::vector<CSASDataElement>* Array)
	{
		for (tList<T>::Iterator Itr = List->Begin(); !Itr.End() && Itr.Get(); ++Itr)
		{
			Array->push_back(Itr.Get());
		}
	}

	BEGIN_CSASCOMMAND_PARAMINFO(GetDataHandlerFormList, 1)
	{
		{ "Form Type", CSASDataElement::kParamType_Numeric }
	};
	BEGIN_CSASCOMMAND_HANDLER(GetDataHandlerFormList)
	{
		double FormType = 0;

		if (!EXTRACT_CSASARGS(&FormType))
			return false;
		else if (FormType < TESForm::kFormType_GMST || FormType > TESForm::kFormType_TOFT)
			return false;
		...
		case TESForm::kFormType_Global:
			AddFormToCSASArray(&_DATAHANDLER->globals, &ArrayData);
			break;
		case TESForm::kFormType_Class:
			AddFormToCSASArray(&_DATAHANDLER->classes, &ArrayData);
			break;
		case TESForm::kFormType_Faction:
			AddFormToCSASArray(&_DATAHANDLER->factions, &ArrayData);
			break;
		case TESForm::kFormType_Hair:
			AddFormToCSASArray(&_DATAHANDLER->hairs, &ArrayData);
			break;
		case TESForm::kFormType_Eyes:
			AddFormToCSASArray(&_DATAHANDLER->eyes, &ArrayData);
			break;
		case TESForm::kFormType_Race:
			AddFormToCSASArray(&_DATAHANDLER->races, &ArrayData);
			break;
		case TESForm::kFormType_Script:
			AddFormToCSASArray(&_DATAHANDLER->scripts, &ArrayData);
			break;
		case TESForm::kFormType_LandTexture:
			AddFormToCSASArray(&_DATAHANDLER->landTextures, &ArrayData);
			break;
		case TESForm::kFormType_Enchantment:
			AddFormToCSASArray(&_DATAHANDLER->enchantmentItems, &ArrayData);
			break;
		case TESForm::kFormType_Spell:
			AddFormToCSASArray(&_DATAHANDLER->spellItems, &ArrayData);
			break;
		case TESForm::kFormType_BirthSign:
			AddFormToCSASArray(&_DATAHANDLER->birthsigns, &ArrayData);
			break;
		case TESForm::kFormType_Weather:
			AddFormToCSASArray(&_DATAHANDLER->weathers, &ArrayData);
			break;
		case TESForm::kFormType_Climate:
			AddFormToCSASArray(&_DATAHANDLER->climates, &ArrayData);
			break;
		case TESForm::kFormType_WorldSpace:
			AddFormToCSASArray(&_DATAHANDLER->worldSpaces, &ArrayData);
			break;
		case TESForm::kFormType_Topic:
			AddFormToCSASArray(&_DATAHANDLER->topics, &ArrayData);
			break;
		case TESForm::kFormType_Quest:
			AddFormToCSASArray(&_DATAHANDLER->quests, &ArrayData);
			break;
		case TESForm::kFormType_AnimObject:
			AddFormToCSASArray(&_DATAHANDLER->objectAnios, &ArrayData);
			break;
		case TESForm::kFormType_CombatStyle:
			AddFormToCSASArray(&_DATAHANDLER->combatStyles, &ArrayData);
			break;
		case TESForm::kFormType_LoadScreen:
			AddFormToCSASArray(&_DATAHANDLER->loadScreens, &ArrayData);
			break;
		case TESForm::kFormType_WaterForm:
			AddFormToCSASArray(&_DATAHANDLER->waterForms, &ArrayData);
			break;
		case TESForm::kFormType_EffectShader:
			AddFormToCSASArray(&_DATAHANDLER->effectShaders, &ArrayData);
			break;
		case TESForm::kFormType_Package:
			AddFormToCSASArray(&_DATAHANDLER->packages, &ArrayData);
			break;
	...
	DEFINE_CSASCOMMAND(GetDataHandlerFormList, "Returns the loaded list of forms of the passed type", CSASDataElement::kParamType_Array, 1);
}