...
namespace ConstructionSetExtender
{
	namespace FormUndoStack
	{
		...
		UInt32 CSEFormUndoProxy::GetFormID( void ) const
		{
			return Parent->formID;
		}

		UInt8 CSEFormUndoProxy::GetType( void ) const
		{
			return Parent->formType;
		}

		const char* CSEFormUndoProxy::GetTypeString( void ) const
		{
			return Parent->GetTypeIDString();
		}

		void CSEFormUndoProxy::Undo( void )
		{
		...
		CSEFormUndoStackOperator::~CSEFormUndoStackOperator()
		{
			;//
		}

		bool CSEFormUndoStackOperator::GetIsFormTypeUndoable( UInt8 Type )
		{
			switch (Type)
			{
			// FormEdit forms
...
//			case TESForm::kFormType_Quest:				### up is down with this form's UI code! too screwed up to fix/enhance
		...
		bool CSEFormUndoStackOperator::PreUndoRedoCallback( bgsee::BGSEEFormUndoProxy* Proxy, bgsee::BGSEEFormUndoProxy** OutAltProxy )
		{
			...
			CSEFormUndoProxy* Current = dynamic_cast<CSEFormUndoProxy*>(Proxy);
			SME_ASSERT(Current);

			*OutAltProxy = new CSEFormUndoProxy(Current->Parent);
			Result = true;

			return Result;
		}

		void CSEFormUndoStackOperator::PostUndoRedoCallback( void )
		{
		...
		{
			bool ComponentInitialized = BGSEEUNDOSTACK->Initialize(new CSEFormUndoStackOperator());
