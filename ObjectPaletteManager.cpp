...
namespace cse
{
	namespace objectPalette
	{
		...
		const char*										ObjectPaletteManager::kPaletteFileExtension = "cseopal";
		const bgsee::ResourceLocation	ObjectPaletteManager::kRepositoryPath(CSE_OPALDEPOT);
...
#define IDC_OBJECTPALETTE_LISTSTATETIMERID			0x202
		...
		INT_PTR CALLBACK ObjectPaletteManager::DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
		{
		...
		int CALLBACK ObjectPaletteManager::SortComparator(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
		{
		...
		void ObjectPaletteManager::InitializeDialog(HWND Dialog)
		{
		...
		void ObjectPaletteManager::DeinitializeDialog(HWND Dialog)
		{
			HWND LoadedObjectsList = GetDlgItem(Dialog, IDC_PALETTEOBJECTS_LOADED);
			HWND SelectedObjectsList = GetDlgItem(Dialog, IDC_PALETTEOBJECTS_SELECTION);
			HWND BaseFormList = GetDlgItem(Dialog, IDC_OBJECTPALETTE_BASEOBJECT);
		...
		void ObjectPaletteManager::RefreshObjectList()
		{
		...
		void ObjectPaletteManager::UpdateSelectionList()
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::RemoveSelection()
		{
		...
		void ObjectPaletteManager::NewPalette()
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::SavePalette(bool NewFile)
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::LoadPalette()
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::MergePalette()
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::UpdatePreview()
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::HandleDragDrop()
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::EnableControls(bool State) const
		{
			SME_ASSERT(MainDialog);
		...
		void ObjectPaletteManager::Close()
		{
			if (MainDialog)
				DestroyWindow(MainDialog);
		}

		bool ObjectPaletteManager::GetFormTypeAllowed(UInt8 Type) const
		{
			switch (Type)
			{
		...
		bool ObjectPaletteManager::ShowFileDialog(bool Save, std::string& OutPath, std::string& OutName)
		{
		...
		bool ObjectPaletteManager::GetBaseHandle(PaletteObject* Data, PaletteObjectHandleT& Out) const
		{
			SME_ASSERT(Data);
		...
		bool ObjectPaletteManager::RemoveLoadedObject(PaletteObject* Data)
		{
			SME_ASSERT(Data);
